...
public class Exercise04_Cards {
    ...
    */
    ...
    public String[] discardFirstCard(String[] hand) {
        String[] newHand = new String[4]; for (int i = 1; i < hand.length; ++i) {
            newHand[i - 1] = hand[i];
        } return newHand;
    }
    ...
    Examples:
    discardTopCard(["8-D", "10-H", "J-C", "8-D", "6-S", "Q-C", "2-D"]) → ["10-H", "J-C", "8-D",
    "6-S", "Q-C", "2-D"]
    ...
    public String[] discardTopCard(String[] remainingDeck) {
        if (remainingDeck.length <= 1) {
            // return new array with no elements
            return new String[0];
        }

        // this will determine the new array size
        int newValue = remainingDeck.length - 1; String[] newDeck = new String[newValue];
        for (int i = 1; i < remainingDeck.length; i++) {
            newDeck[i - 1] = remainingDeck[i]; // [i-1] == 0
        } return newDeck;
    }
}
