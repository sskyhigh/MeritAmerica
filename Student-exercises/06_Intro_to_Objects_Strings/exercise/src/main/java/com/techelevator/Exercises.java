...
public class Exercises {
    ...
    public String makeTags(String tag, String word) {
        return "<" + tag.concat(">").concat(word).concat("</").concat(tag).concat(">");
    }

    /*
     Given an "out" string length 4, such as "<<>>", and a word, return a new string where the
     word is in the
     middle of the out string, e.g. "<<word>>". Note: use str.substring(i, j) to extract the
     String starting
    ...
    public String extraEnd(String str) {
        ...
        return newWord.concat(newWord).concat(newWord);
    }

    /*
     Given a string, return the string made of its first two chars, so the String "Hello" yields
     "He". If the
     string is shorter than length 2, return whatever there is, so "X" yields "X", and the empty
     string ""
    ...
    public String left2(String str) {
        if (str.length() == 2) {
            return str;
        } String firstTwo = str.substring(0, 2); String lastSet = str.substring(2);
        return lastSet.concat(firstTwo);
    }
    ...
    public String right2(String str) {
        ...
        String firstSet = str.substring(0, str.length() - 2); return lastTwo.concat(firstSet);
    }
    ...
    public String nTwice(String str, int n) {
        ...
        String first = str.substring(0, n); String last = str.substring(str.length() - n);
        return first.concat(last);
    }

    /*
     Given a string and an int n, return a string that starts at n and has a length of 2. Note
     that n may or may not be a valid
     location in the string. If n is too low or too high to define a string of length 2, return
     the string's first 2 characters.
     The string length will be at least 2.
     twoChar("java", 0) → "ja"
     twoChar("java", 2) → "va"
     twoChar("java", 3) → "ja"
    ...
    public boolean hasBad(String str) {
        return str.startsWith("bad", 1) || str.startsWith("bad");
    }
    ...
    public String frontTimes(String str, int n) {
        if (str.length() <= 2) {
            return str.repeat(n);
        } String newWord = str.substring(0, 3); return newWord.repeat(n);
    }
    ...
    public String stringBits(String str) {
        char[] ch = str.toCharArray(); StringBuilder sb = new StringBuilder(str.length());
    ...
    public String stringSplosion(String str) {
        StringBuilder sb = new StringBuilder(str.length()); for (int i = 0; i < str.length(); ++i) {
            sb.append(str, 0, i + 1);
        } return sb.toString();
    }
    ...
    /*
     Given a string, return the count of the number of times that a substring length 2 appears in
      the string and
     also as the last 2 chars of the string, so "hixxxhi" yields 1 (we won't count the end
     substring).
     last2("hixxhi") → 1
     last2("xaxxaxaxx") → 1
     last2("axxxaaxx") → 2
    ...
    public int last2(String str) {
        if (str.length() < 2) {
            return 0;
        } int count = 0; String occurrence = str.substring(str.length() - 2);
        for (int i = 0; i < str.length() - 2; ++i) {
            if (str.substring(i, i + 2).equals(occurrence)) {
                ++count;
            }
        } return count;
    }
    ...
    public String stringX(String str) {
        if (str.length() < 3) {
            return str;
        } char firstWord = str.charAt(0); char lastWord = str.charAt(str.length() - 1);

        String middleWord = str.substring(1, str.length() - 1);
        middleWord = middleWord.replace("x", "");
