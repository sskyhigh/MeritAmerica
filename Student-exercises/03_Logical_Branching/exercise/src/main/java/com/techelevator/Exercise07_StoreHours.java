...
public class Exercise07_StoreHours {
    ...
    /*
    Shelia's Seashell Store has a new website. Shelia wants to display if the store is open when
    someone views the website.
    ...
    The store is open if the current hour is equal to or greater than the open time and less than
     the closing time.
    ...
    Her store is open between 8 am (hour 8) and 5 pm (hour 17) on Monday (day M), Wednesday (day
    W), and Friday (day F).
    Implement the logic to determine if the store is open based on the current hour and the
    current day.

    NOTE: This problem uses a char to represent the current day of the week. You can use the
    following key:
        Monday = 'M'
        Wednesday = 'W'
        Friday = 'F'
    ...
    public boolean isStoreOpen(int currentHour, char currentDay) {
        return currentHour >= 8 && currentHour < 17 && (currentDay != 'S');
    }
    ...
    open until 8 pm (hour 20) and is also
    open Saturday (day S) from 9 am (hour 9) to 3 pm (hour 15).

    Implement the logic to determine if the store is open based on the current hour, the current
    day, and if it's summer.
