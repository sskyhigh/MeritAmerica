    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (userChoice.equals("1")) {
                ...
                // Step Two:
                System.out.println("Added " + newWall + " wall - " + newWall.getArea() + " square"
                        + " feet");
                walls.add(newWall);
                ...
                for (int i = 0; i < walls.size(); i++) {
                    System.out.printf("Wall %d: %s - %d square ft%n", i + 1,
                            walls.get(i), walls.get(i).getArea());
                    totalArea = totalArea + walls.get(i).getArea();
                    colorChoiceAreas.put(walls.get(i).getColor(),
                            colorChoiceAreas.getOrDefault(walls.get(i).getColor(), 0) + walls.get(i).getArea());
                }
                ...
                for (Map.Entry<String, Integer> colorEntry : colorChoiceAreas.entrySet()) {
                    ...
                    System.out.println(colorEntry.getKey() + " Paint Required: " + gallonsRequiredForColor + " gallons");
                }
                return;
            }
        }
