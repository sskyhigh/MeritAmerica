...
public class SearchEngine {
    ...
    public void indexFiles() throws SearchEngineException, Exception {
        ...
        List indexFiles = sd.getFiles();
        for(int i = 0; i<indexFiles.size(); ++i){
            BufferedReader reader = new BufferedReader(new FileReader(sd.getFolder() + "/" + indexFiles.get(i)));
            String line;
            while((line= reader.readLine())!=null){
                indexWords(i, line);
            }
        }
        String loggedWords = indexedWordsToString();
        TELog.log(loggedWords);
    }
            ...
            if (searchWords.length == 1) {
                ...
                if (distances.size() > 0) {
                    for (int currentWordIndex = 1; currentWordIndex < searchWords.length - 1; currentWordIndex++) {
                        List<WordDistance> nextDistances =
                                getDistances(searchWords[currentWordIndex],
                                        searchWords[currentWordIndex + 1]);
