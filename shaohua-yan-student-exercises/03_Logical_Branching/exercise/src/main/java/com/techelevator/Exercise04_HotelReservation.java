...
public class Exercise04_HotelReservation {
    ...
    The problems below ask you to implement the logic for determining a guest's total amount for
    their stay.
     */
    ...
    public double calculateStayTotal(int numberOfNights) {
        return numberOfNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE ? DAILY_RATE * numberOfNights :
                DISCOUNT_RATE * numberOfNights;
    }
    ...
    public double calculateStayTotal(int numOfTotalNights, boolean includesParking) {
        ...
        if (includesParking) {
            if (numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE) {
                fee = (DAILY_RATE * numOfTotalNights) + (PARKING_RATE * numOfTotalNights);
            } else {
                fee = (DISCOUNT_RATE * numOfTotalNights) + (PARKING_RATE * numOfTotalNights);
            }
        } else {
            if (numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE) {
                fee = (DAILY_RATE * numOfTotalNights);
            } else {
                fee = (DISCOUNT_RATE * numOfTotalNights);
            }
        }
        return fee;
    }
     ...
     */
    public double calculateStayTotal(int numOfTotalNights, boolean includesParking,
                                     boolean includesLateCheckout) {
        double fee = 0;
        if (includesParking) {
            if (includesLateCheckout) {
                if (numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE) {
                    fee = ((DAILY_RATE * numOfTotalNights) + (PARKING_RATE * numOfTotalNights) + LATE_CHECKOUT_FEE);
                } else {
                    fee = (DISCOUNT_RATE * numOfTotalNights) + (PARKING_RATE * numOfTotalNights) + LATE_CHECKOUT_FEE;
                }
            } else {
                if (numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE) {
                    fee = ((DAILY_RATE * numOfTotalNights) + (PARKING_RATE * numOfTotalNights));
            ...
            if (includesLateCheckout) {
                if (numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE) {
                    fee = ((DAILY_RATE * numOfTotalNights) + LATE_CHECKOUT_FEE);
                } else {
                    fee = ((DISCOUNT_RATE * numOfTotalNights) + LATE_CHECKOUT_FEE);
                }
            } else {
                if (numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE) {
                    fee = ((DAILY_RATE * numOfTotalNights));
