...
public class Exercise05_AcceptPackage {
    ...
    Implement the logic to determine if Scamper Shipping accepts a package based on the weight
    provided.
    ...
    Scamper decides that in addition to the package being no more than 40 pounds, they must also
    limit its size.
    Scamper won't deliver packages greater than four cubic feet (6912 cubic inches).

    Implement the logic to determine if Scamper Shipping accepts a package based on the weight,
    length, width, and height provided.
     ...
     */
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches,
                                 int heightInches) {
        return weightPounds <= 40 && lengthInches * widthInches * heightInches <= MAX_CUBIC_INCHES;
    }

    /*
    All was well until a customer showed up with a 16-foot garden hose laid out straight in a
    2x2x194 inch shipping box.
    ...
    no single dimension can be greater than five and a half feet (66 inches) without paying a
    surcharge.
     ...
     */
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches,
                                 int heightInches, boolean isSurchargePaid) {
        boolean check = true;
        int total = widthInches * lengthInches * heightInches;
        if (weightPounds <= MAX_WEIGHT_POUNDS) {
            if (total < MAX_CUBIC_INCHES) {
                if (lengthInches > 66 || widthInches > 66 || heightInches > 66) {
                    check = isSurchargePaid;
                }
            } else {
                check = false;
            }
