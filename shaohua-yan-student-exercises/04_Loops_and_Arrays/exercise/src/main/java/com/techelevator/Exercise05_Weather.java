...
public class Exercise05_Weather {
    ...
    */
    public int hottestDay(int[] dailyHighs) {
        int highest = dailyHighs[0];
        int temp = 0;
        for (int i = 1; i < dailyHighs.length; ++i) {
            if (dailyHighs[i] > highest) {
                highest = dailyHighs[i];
            }
        }
        return highest;
    }
    ...
    public int[] fixTemperatures(int[] temperatures) {
        int[] newValue = new int[temperatures.length];
        for(int i = 0; i<newValue.length; ++i){
            if(i%2 == 0){
                newValue[i] = temperatures[i]+ 2;
            }
            else{
                newValue[i] = temperatures[i];
            }
        }
        return newValue;
    }
