...
public class Tutorial {
    ...
    public static void main(String[] args) {
        ...
        int highestTempValue = forecastTemperatures[0];
        int highestTempIndex = 0;

        for(int j = 1; j< forecastTemperatures.length; ++j){
            if(forecastTemperatures[j] > highestTempValue){
                highestTempValue = forecastTemperatures[j];
                highestTempIndex = j;
            }
        }
        System.out.println("The highest temperature is: " + highestTempValue);
        System.out.println("The highest temperature is in " + (highestTempIndex + 1)+ " days");
    }
}
